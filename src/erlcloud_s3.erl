...
-spec signature(aws_config(), string(), string(), string(), proplist(), proplist(), string()) -> string().
signature(Config, Path, Date, Region, QueryParams, Headers, Payload) ->
    Service = "s3",
    CredentialScope = erlcloud_aws:credential_scope(Date, Region, Service),
    {CanonicalRequest, _} = erlcloud_aws:canonical_request(get, Path, QueryParams, Headers, Payload),
